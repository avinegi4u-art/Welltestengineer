...
| `flowsim-pro/` | **FlowSim Pro v1.1** — steady-state multiphase flow (Beggs-Brill VLP, nodal analysis, tubing/flowline selectors). FastAPI + Next.js. **One-click:** double-click `flowsim-pro/Open FlowSim Pro.bat` (Windows) or run `flowsim-pro/open-flowsim-pro.sh`. See `flowsim-pro/README.md` |

Open any `.html` file in a browser — no build step required. For FlowSim Pro, use the one-click launcher (or start backend + frontend — see its README).
